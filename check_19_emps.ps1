...
# Check if there is an action to get all raw Name sheet rows or test specific employee IDs
# Let's inspect the 19 unregistered IDs and see if any of them have remarks or if any other IDs were excluded
$unregistered19 = @(
...
Write-Host "Checking 19 unregistered employees via getEmployeeData..."
foreach ($id in $unregistered19) {
    ...
    if ($r.success) {
        $d = $r.data
        Write-Host "$id : $($d.firstName) $($d.lastName) | Dept: $($d.department) | Right: '$($d.checkupRight)' | Remark: '$($d.remark)'"
    } else {
        Write-Host "$id : Error or not found: $($r.error)"
    }
}
